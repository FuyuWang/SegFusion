...
class Timeloop(object):
    def __init__(self, in_config_path='./', out_config_path='./out_config', accelerator='simba',
    ...
    def get_map_config(self, program):
        ...
        for level in range(1, self.num_buffer_level+1):
            ...
            if level == self.num_buffer_level:
                # permutation = 'SRCKHQPN'
                permutation = 'SRCKQPNH'
    ...
    def run_timeloop(self, dimension,  sol, pool_idx=0, use_IO=False, fitness_obj=['latency']):
    ...
    def run_config(self, filename):
        ...
        if report_file.exists():
            ...
            # # print(len(memory_latency), len(memory_energy), len(memory_bandwidth))
            # # print(memory_latency, memory_bandwidth[2:4], memory_bandwidth[4:])
            # # smem_access = memory_latency[3] * (memory_bandwidth[2] + memory_bandwidth[3])
            # # smem_energy = memory_energy[3] + memory_energy[4]
            ...
            #
            # # dram_access = memory_latency[4] * (memory_bandwidth[4] + memory_bandwidth[5] + memory_bandwidth[6])
            # # dram_energy = memory_energy[5] + memory_energy[6] + memory_energy[7]
            # # dram_energy_ratio = dram_energy / dram_access
            ...
            # status_dict['cycles'] = np.array(memory_latency[:-1]).max()
            # status_dict['energy'] = np.array(memory_energy).sum() * 1e-6
